...
public class UserNameRecognizer {
	 ...
	 * 
	 * @version 2.00		2026-09-16	Updated username rules for TP1: alphabetic first character, updated separators (include &), max-length 32 characters
	 * 
	 */
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 1: 
				...
				//  2: a separator ('-', '_', '.', '&') that transitions to state 2 
				...
				// ('-', '_', '.', '&') -> State 2
				else if (currentChar == '-' || currentChar == '_' || currentChar == '.' || currentChar == '&') {	// Check for a separator
					nextState = 2;
